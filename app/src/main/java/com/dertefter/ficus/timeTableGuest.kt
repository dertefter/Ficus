...
class timeTableGuest : Fragment(R.layout.timetable_fragment_guest) {
    ...
    fun lessons(week_: Int?) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try{
                val response = service.getTimetableGuest(AppPreferences.group, "true")
                withContext(Dispatchers.Main) {
                    if (response.isSuccessful) {
                        ...
                        if (week_ == null){
                            week_number = Calendar.getInstance().get(Calendar.WEEK_OF_YEAR) - 34
                            if (weeks_view?.childCount == 0){
                                for (w in week_number..week_number+6){
